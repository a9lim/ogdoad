//! The Minkowski--Siegel mass formula and an explicit Leech lattice.
//!
...
//! closed form in Bernoulli numbers ([`mass_even_unimodular`]):
//!
//! ```text
//! mass(n) = (|B_{n/2}| / n) · ∏_{j=1}^{n/2−1} |B_{2j}| / (4j).
//! ```
//!
//! At `n = 8` this is `1/696729600 = 1/|Aut(E_8)|`. Bernoulli values are generated
//! by an exact recurrence. The `i128` rational backend returns `None` beyond rank
//! 24, where its fixed-width arithmetic is insufficient.
//!
...
//! `B` by Hermite normal form, with `Gram = B·Bᵀ / 8`. Tests check that it is rank
//! 24, even, unimodular, and rootless; Niemeier's classification then identifies
//! it with the Leech lattice.
//! Its automorphism group is the Conway group `Co₀ = 2·Co₁`; [`LEECH_AUT_ORDER`]
//! is computed from its standard prime-power factorization.
//!
...
/// Multiply two `(num, den)` fractions via [`Rational::checked_mul`], which owns
/// the cross-gcd reduction; kept as a `(i128, i128)` kit so callers here don't
...
///
/// This is the "second Bernoulli number" convention (`B_n^+`, the one the
/// classical Akiyama–Tanigawa transform naturally produces): it agrees with the
/// more common `B_1 = −1/2` convention at every even index and disagrees only at
/// `n = 1`. Every consumer of this source ([`mass_even_unimodular`] and the
/// Eisenstein normalizing constants in
/// [`modular`](crate::forms::integral::modular)) reads only even indices, so the
/// sign choice at `n = 1` is inert downstream — documented here rather than
/// papered over with a special-cased negation.
...
mod tests {
    ...
    fn bernoulli_one_is_the_plus_convention() {
        // Pin the sign this source actually produces at the one odd index where
        // the two classical Bernoulli conventions disagree: B_1^+ = +1/2 (the
        // Akiyama–Tanigawa transform's native convention), not B_1^- = -1/2.
