//! Brauer--Wall classes of the supported Clifford-algebra coefficient fields.
//!
...
//! the implementations below compute the resulting class on these domains:
//!
//!   * **`BW(ℝ) ≅ ℤ/8`** — the class of `Cl(p,q)` is
//!     `s = (q − p) mod 8`, and `⊗̂` adds indices.
...
//!     Brauer component is the Clifford invariant `c(q)`, while the
...
//!     (`q ≡ 1 mod 4`). Tests verify the group law against direct sums.
...
//!
//! Tests check `[Cl(V) ⊗̂ Cl(W)] = [Cl(V)] + [Cl(W)]` against metric direct sums.
...
pub enum BrauerWallClass {
    ...
    OddChar {
        ...
        field_order: u128,
        /// Whether `-1` is a nonsquare.
        kappa: u128,
        /// Dimension parity.
        e0: u128,
        /// Signed-discriminant square class.
        sclass: u128,
    },
...
/// * `clifford_brauer_class`: the ungraded Clifford invariant `c(q)`.
///
...
impl RationalBrauerWallClass {
    ...
    /// The ungraded Clifford invariant `c(q)`.
...
mod tests {
    ...
    fn bw_real_is_the_bott_clock() {
        // `bw_class_real` agrees with the `classify_real` index
        // s = (q − p) mod 8.
        for p in 0..5usize {
            for q in 0..5usize {
    ...
    // Function-field correction-table sweep over every n mod 8 branch.
