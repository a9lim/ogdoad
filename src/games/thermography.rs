...
fn req(a: &Rational, b: &Rational) -> bool {
...
impl Pl {
    ...
    fn cleaned(mut self) -> Pl {
        // dedup equal t (continuity ⇒ equal value)
        ...
        for p in dedup {
            while out.len() >= 2 {
                let a = &out[out.len() - 2];
                let b = &out[out.len() - 1];
                // collinear iff (b.v − a.v)(p.t − b.t) == (p.v − b.v)(b.t − a.t)
...
/// Sorted-unique union of the breakpoint abscissae of `f` and `g`.
...
/// The crossing temperature of `f` and `g` strictly inside `(a, b)`, where both
/// are linear, or `None` if they don't meet there.
fn cross(f: &Pl, g: &Pl, a: &Rational, b: &Rational) -> Option<Rational> {
    ...
    // solve fa + (fb−fa)s = ga + (gb−ga)s  ⇒  s = (ga−fa)/((fb−fa)−(gb−ga))
