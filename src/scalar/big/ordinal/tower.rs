...
fn finite_excess(u: u128) -> Option<u128> {
    ...
    // The next ordinary singleton row has a self-contained degree-2420 binary-field
    // certificate in `experiments/ordinary_727_certificate.py`.
    if u == 727 {
        return Some(1);
    }
...
/// some owed `α_u` is past the verified table (`u > 727`).
...
/// carry needs an excess `α_u` past the verified table (`u > 727`). Distributes over CNF
...
mod tests {
    ...
    #[test]
    fn local_727_certificate_extends_the_tower_one_more_row() {
        assert_eq!(multiplicative_order_two_mod_prime(727), Some(121));
        assert_eq!(q_set(121).as_deref(), Some(&[121][..]));
        assert_eq!(finite_excess(727), Some(1));

        let chi121 = Ordinal::omega_pow(Ordinal::monomial(
            fin(odd_prime_place(11).expect("11 is prime")),
            11,
        ));
        assert_eq!(alpha_ordinal(727), Some(chi121.nim_add(&fin(1))));
    }
    ...
    fn finite_excess_boundary_returns_none_past_prime_727() {
        ...
        // Local exact certificates extend the boundary through places 126 and 127,
        // two rows beyond the OEIS b-file.
        assert_eq!(place_prime(125), 709);
        assert_eq!(place_prime(126), 719);
        assert_eq!(place_prime(127), 727);
        assert_eq!(place_prime(128), 733);
