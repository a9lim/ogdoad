...
//! `φ_{ω+1}` (`< ω³`) coordinate helpers, and the CNF canonicalizer
...
impl Ordinal {
    ...
    /// (the ordinal lies outside this coordinate window).
    ...
    /// Nim-multiplication across the Conway--Lenstra--DiMuro prime-power
    /// generator tower. Non-scalar excesses branch a Kummer carry into a sum and
    /// recurse strictly downward by prime place.
    ///
...
mod tests {
    ...
    fn omega_cubed_is_two() {
        // Conway/DiMuro: ω is the nim cube root of 2. This is the
    ...
    fn f4_adjoin_omega_is_a_field() {
        ...
        // independent reduction oracle, so this keeps exhaustive pair facts and checks the
    ...
    fn tower_multiplication_ring_axioms() {
        ...
        // 0/1-combination of ω^0..ω^8). The decisive check is the
    ...
    fn multiplication_reaches_past_omega_omega() {
        ...
        assert_eq!(p, fin(4));
        // The first branching carry: χ_7 = ω^(ω²), and (χ_7)⊗⁷ = α_7 = ω + 1 (non-scalar —
        ...
        assert_eq!(q, omega.nim_add(&fin(1))); // ω + 1
                                               // Rows through prime 727 are represented; ≥ ω^(ω^ω) is out of range.
