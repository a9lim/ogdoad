...
impl Ordinal {
    ...
    /// carries only at primes `≤ 719` (the 126 OEIS A380496 rows through 709 plus the
    /// locally certified 719 row; see the `tower` module's `finite_excess`).
    ///
    ...
    /// an `α_u` past the verified table (`α_727` and beyond, the first unsupported row)
    /// — the staged boundary.
...
mod tests {
    ...
    fn multiplication_reaches_past_omega_omega() {
        ...
        // stay at primes ≤ 719 — well past ω^ω. Spot-checks of verified landmarks
        ...
        assert_eq!(q, omega.nim_add(&fin(1))); // ω + 1
                                               // The finite-row boundary now sits past prime 719, and ≥ ω^(ω^ω) is out of range.
