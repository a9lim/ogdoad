...
pub(crate) fn reduce_integer_vector(v: &mut [i128], rows: Vec<Vec<i128>>) {
    let rows = normalize_relation_rows(rows);
    reduce_integer_vector_normalized(v, &rows);
}

/// Reduce `v` modulo rows already returned by [`normalize_relation_rows`].
pub(crate) fn reduce_integer_vector_normalized(v: &mut [i128], rows: &[Vec<i128>]) {
    for row in rows {
