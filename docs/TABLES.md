...
| Finite Lenstra excess integers `m_u`, odd primes `3..=727` | `src/scalar/big/ordinal/tower.rs::finite_excess` | Yes. | OEIS A380496 ("Lenstra excess of the n-th odd prime") supplies 126 rows through `p=709`, diffed in full against the vendored b-file.  The rows `m_719=m_727=1` are supplied separately by local exact certificates: the crossed-tower payloads at 719 and the self-contained degree-2420 binary-field check at 727.  Tests keep source provenance separate and check both reconstructions.  The deliberate rowwise cutoff is 727; `p=733` is only the operational refusal boundary while research targets the global theorem.  `alpha_u` is assembled from `ord_u(2)`, `Q(f(u))`, and this finite integer. |
