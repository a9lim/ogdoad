...
| Finite Lenstra excess integers `m_u`, odd primes `3..=719` | `src/scalar/big/ordinal/tower.rs::finite_excess` | Yes. | OEIS A380496 ("Lenstra excess of the n-th odd prime") supplies 126 rows through `p=709`, diffed in full against the vendored b-file.  The next row `m_719=1` is supplied separately by the locally replayable crossed-tower certificate.  Tests keep source provenance separate, check the 719 reconstruction, and move the operational boundary to `p=727`.  `alpha_u` is assembled from `ord_u(2)`, `Q(f(u))`, and this finite integer. |
