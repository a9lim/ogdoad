...
- **`Z`.** Separate component norms lose a synchronized Frobenius phase. The
  ...
  unique adjacent selected roots. This is exactly the `R`-th Frobenius power
  of the earlier equation `Phi((v^P)^K)/Phi(v^K)=t^(3K-2)`, so injectivity of
  Frobenius makes the two equations equivalent. An exact genuine
  adjacent-Conway example at `(ell,R,K,eta)=(257,4,53,213)` satisfies the
  inverse-normalized equation, showing that adjacent selection plus the
  full-lower inverse does not exclude it. More strongly, an exact degree-48
  two-edge model with proper `ell=193`, full adjacent torus orders, and the
  complete inverse/Jacobi/trace packet satisfies the marked equation. Its
  full four-state equality `S_g(a)=a^K` fails, precisely isolating `QFC4` as
  the independent selected content. Thus the nonhomogeneous seam is a full
  oriented fibre, and the inverse congruence adds no equation.
  ...
  first relative-trace refinement also stops exactly at distinguished
  ancestry. Its collision equation factors into a half-field axis and a
  relative norm-one axis. An explicit degree-16/full-order predecessor has
  an irreducible exact Conway resultant in the proper `F_5/641` stratum, and
  two distinct roots of the resulting `641`-root polynomial have identical
  relative trace. Thus lower maximality, the exact one-edge transition, and
  nine matching absolute moments do not make a collision descend to lower
  failure. The model is not on the distinguished path: imposing the complete
  ancestry chain forces the predecessor back into the selected Frobenius
  orbit. Consequently only a genuinely full-ancestry collision-descent
  theorem remains possible. The
  ...
  maximality proof. The strongest applicable invariant-`PGL_2` high-order
  theorem also collapses to the same half-Frobenius inversion. Over every
  Conway subfield, genericity forces the projective matrix to have order two
  and to be inversion; the resulting published lower bound is `O(2^n)` and
  is weaker than the elementary degree bound. Translating to
  `c_n^(-1)=w_n+1` only conjugates the apparent upper-unipotent matrix back
  to inversion, so the favorable independent-row branch is unavailable.
  Recursive rebasing weakens rather than amplifies the estimate. The rational
  dynamics is likewise exact but non-Lattès:
  ...
  extension, not an ordinary Massey product or nilpotent quotient. Enlarging
  within multiplicative Conway ancestry still adds no coordinate. Modulo
  `ell`-th powers the complete ancestry span is the fixed lower span plus the
  single anti-invariant line generated by `[W_n]`; every independent extra
  ancestry radical reduces to a lower unit, gives only
  `D_(2*ell) times C_ell`, and splits at the selected prime above two. The
  sole nonsplit sign extension lengthens the same rotation to order `ell^2`.
  The natural characteristic-two additive enlargement is equally acyclic:
  on `V=(F_(q^2),+)`, with rotation by `zeta_ell` and reflection by
  `q`-Frobenius, every `H^r(D_(2*ell),V)` vanishes. The literal Conway affine
  cocycle is a coboundary, and its norm determinant is exactly
  `a_(n-1)*(H+H^(-1))`, nonzero precisely when the original phase `H` is
  nontrivial. Under failure the local unramified `H^1` becomes all of `V`, so
  the additive extension accommodates rather than contradicts failure. A
  larger-extension proof must therefore import a ramified or non-ancestral
  class with an independently normalized rotation-fixed quotient. The
