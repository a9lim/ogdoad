...
\item that multiples of a generator killed by the hyperbolic relation
\(2L=0\) depend only on parity, while a homomorphism to \(\Integers/2\)
detecting \(L\) proves noncollapse.
\end{enumerate}
The kernel therefore checks the algebraic core of
Theorem~\ref{thm:hesscollapse} and the cancellation step in the freezing
obstruction, together with the universal group-theoretic step of
Theorem~\ref{thm:ringcollapse}.  Classical diagonalization, the
Cantor-normal-form identification, the unique-base theorem, finite graph
decomposition, supertropical companion classification, and standard
thermograph facts remain proved here or cited external mathematics, not Lean
theorems.
