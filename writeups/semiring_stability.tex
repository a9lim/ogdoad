...
\title{Quadratic Stability without Subtraction:\\
Unique-Base Semirings, Scalar Extension, and Thermograph Walls}
...
\begin{abstract}
Over a semiring, polarization, cancellation, and group completion are distinct
operations.  We separate them by working with finite free quadratic pairs
$(q,b)$ over semirings with unique bases.  The pair is encoded by a weighted
coefficient graph, monomial basis change acts by a unit gauge, and orthogonal
sum is disjoint union.  Consequently the isometry monoid is free commutative
on connected gauge-orbit types.  Regular pairs split into rank-one loops and
rank-two edges, and basic metabolic stabilization deletes exactly the
metabolic edge generators.
...
Witt group is zero.  The all-pairs stable monoid nevertheless remains free on
every connected weighted-graph type except the hyperbolic edge and has an
exact finite decision procedure.
...
functorial only for based forms.  For every field of characteristic different
from two and every rational tangible supervaluation, the universal
basis-independent ring-image quotient is exactly \(\mathbb Z/2\), detected by
rank parity, for both the canonical balanced and polar-companion lifts.

Finally, the same framework tests a proposed game-valued application.
Completing thermograph walls by one global tropical zero produces honest
max-plus and min-plus coefficient semifields, and each wall pair defines a
regular rank-one quadratic pair classified by its gap.  The self-negative hot
switch \(\{1\mid-1\}\) has nonzero gap, so this stable wall class cannot be
additive under disjunctive sum.
\end{abstract}

\section{Introduction}

Classical Witt theory uses subtraction at several logically different points:
to recover a polar form from a quadratic form, to cancel orthogonal summands,
and to pass from isometry classes to a group.  None of these moves is automatic
over a semiring.  The correct first object is therefore a quadratic pair
\((q,b)\),
where
\[
 q(ax)=a^2q(x),\qquad
...
The organizing input is the unique-base theorem of Izhakian, Knebusch, and
Rowen: if the nonzero elements of a semiring are closed under addition and
multiplication, every basis of a free module differs from every other by a
permutation and multiplication by scalar units
\cite[Theorem~1.3]{ikr2016uniquebase}.  Matrix reduction is thereby replaced
by finite graph theory.  Connected components control orthogonal
decomposition, while invertible Gram matrices reduce to involutions of the
unique basis.  This one mechanism governs both the regular theory and its much
larger singular envelope.

We use that mechanism to answer one question in three coefficient worlds:
what does stable quadratic structure retain when subtraction is unavailable?
Hessenberg natural arithmetic gives a rigid, cancellative example;
supertropical arithmetic adds nonunique companions and a meaningful
scalar-extension problem; thermograph walls test whether the resulting stable
classes can carry addition from the Conway game group.
...
The proof has a single spine.  Section~\ref{sec:graphs} constructs the graph
presentation and stable quotient for an arbitrary unique-base semiring.
Sections~\ref{sec:hessenberg} and~\ref{sec:supertropical} specialize it to
Hessenberg and rational supertropical coefficients.
Section~\ref{sec:walls} applies the same rank-one classification to
thermograph walls.  Section~\ref{sec:formal} states exactly which joins are
kernel-checked and which remain cited or paper-level mathematics.

The Hessenberg result is a complete classification and collapse theorem.  The
supertropical block theorem and universal classical ring-image quotient are
complete for the stated rational tangible cover.  The wall result is a no-go
theorem for the natural max/min construction; it does not rule out every
possible synchronized quotient that deliberately kills the exhibited gap
class.

\section{Quadratic pairs over unique-base semirings}\label{sec:graphs}
...
\subsection{Coefficient graphs and cancellation}
...
\subsection{Regular blocks and stabilization}
...
\section{Hessenberg natural arithmetic}\label{sec:hessenberg}
...
\subsection{Polynomial coordinates and companion rigidity}
...
\subsection{Regular collapse and the singular envelope}
...
\section{Rational supertropical quadratic pairs}\label{sec:supertropical}

\subsection{Intrinsic blocks and companion ambiguity}
...
blocks \(H(a_1,a_2;c)\) from Theorem~\ref{thm:blocks}.  The all-pairs stable
group is free abelian on all connected types except the edge types
\[
...
\section{Thermograph walls and additivity}\label{sec:walls}

\subsection{Tropical completion and wall pairs}
...
\subsection{The disjunctive-sum obstruction}
...
\section{Formal verification and implementation scope}\label{sec:formal}

The import-only entry point
\texttt{Ogdoad.Papers.}\allowbreak\texttt{SemiringStability} exposes
the Lean module \texttt{Ogdoad.SemiringQuadratic} \cite{ogdoad2026}, which checks:
\begin{enumerate}
...
\begin{center}
\texttt{hessenbergPolynomial\_regular\_isometric\_hyperbolic},
\end{center}
which shows that bijectivity of the companion adjoint implies even rank and
produces an explicit coefficient calculation together with an actual linear
isometry of quadratic pairs to a canonical orthogonal sum of hyperbolic
planes;
...
Unique bases turn the absence of subtraction from an ambiguity into a
classification tool: stable quadratic pairs become connected weighted graphs
modulo a scalar gauge.  In the Hessenberg fragments this yields both extremes
at once.  The regular Witt theory is zero, while the singular stable envelope
is a large free invariant with a finite decision procedure.  Over the rational
supertropical cover the same theorem gives exact blocks, companion ambiguity,
and metabolic relations.  Coefficientwise supervaluation remembers a chosen
classical basis, but after imposing all source basis changes the universal
classical image is exactly rank parity in characteristic different from two.

Thermograph walls complete the picture.  After the minimal tropical
completion they form honest regular rank-one pairs, and their isometry class
is exactly the wall gap.  Yet the self-negative hot switch makes that nonzero
class incompatible with additivity.  The common lesson is precise: the graph
classification survives without subtraction, but classical basis invariance
and game addition impose further quotient relations that the intrinsic
semiring theory neither supplies nor conceals.
