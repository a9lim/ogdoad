...
\title{Gold--Arf normal play from split-source Witt--FIFO}
\author{a9lim}
\date{August 2026}
...
normal-play game whose $P$-positions are exactly
$\{x:Q_a(x)=0\}$, so Arf is the sign of its second-player win-bias.
...
tautological evaluation. Third, the constructive resolution: a public,
refinement-blind Witt frame turns the strategic support graph into a matching,
while separate isolated source coins retain weight-one access to the original
Grundy-frame diagonal. A local FIFO strategy proves that either seat forces
zero correction parity on every matching-plus-isolates board. A one-move
terminal charge lantern then compiles the forced bit to ordinary normal play.
The resulting fixed rule is refinement-uniform, uses at most one weight-one
refinement query per transition, and has counterfactually disjoint optimal
actions whenever the polar rank is positive.
\end{abstract}
...
proved here is structural rather than extrapolated from that census; its
matching-board strategy and normal-play compiler are independently
kernel-checked in \path{formal/Ogdoad/FifoMatching.lean} and
...
\item[\textbf{N3cf}] (\emph{counterfactual optimal-action dependence --
anti-clock}) for every positive-polar-rank instance $(m,B,q)$ there are a
refinement $q'$ of the same $B$ and a common reachable position $s$ which is
winning for the player to move in both games, but whose two nonempty sets of
winning moves $W_q(s)$ and $W_{q'}(s)$ are disjoint.
\end{enumerate}
A rule with a reachable position having both a winning and a losing move is
called \emph{decision-nondegenerate}. Rank-zero instances are required to
satisfy N1--N2 but are excluded from N3cf: adding a strategically irrelevant
polar gadget there would reinstate the escape-edge defect by definition.
\end{definition}
...
N1+N2 alone, without N3cf, admit pure transport clocks in every semantics,
...
characteristic-$2$ form. Hence N3cf is load-bearing: it carries the entire
residual content of ``natural''.
\end{theorem}

\begin{remark}[Why outcome-critical N3 is replaced]\label{rem:N3}
The \emph{escape-edge} construction -- a clock plus one dominated $q$-blind
gadget edge from every nonzero position -- passes N1, N2, and the old N3 (and
a density-strengthened N3$^{+}$) while being morally a clock
(\path{experiments/gold/skeptic_escape_edge_attack.py}). The natural
dominance-pruned mixed-successor repair is structurally impossible: at an
$N$-position pruning retains exactly the options to $P$, while every option
of a $P$-position is $N$. Thus all retained successors at any position have
one outcome class. N3cf instead compares the \emph{winning action sets} in
two counterfactual refinements at the same state. It is invariant under
deleting dominated losing options, and a dead escape edge cannot affect it.
The two-class impossibility and the Boolean action-swap identity are
kernel-checked in \path{formal/Ogdoad/GoldSemantics.lean}.
\end{remark}

\paragraph{Calibration (implemented and tested at the stated instances).}
Under F1--F3 + N1--N2: the T2 spin-flip rule of
Section~\ref{sec:constructions} fails N3cf (its one forced winning move does
not counterfactually swap); the pending-marker clock fails N3cf;
...
The candidates named here live in \texttt{experiments/gold/}. The split-source
Witt--FIFO rule below satisfies N3cf for every positive-rank instance by
theorem, not census.
...
\section{Resolution: the split-source Witt--FIFO game}
\label{sec:wittfifo}
...
part and separate weight-one refinement sources.
...
\begin{quote}
\textbf{Rule (split-source \textsc{witt-fifo}).}
...
Load in addition an isolated source coin $S_j$ for each original coordinate
$x_j=1$; closing $S_j$ reads the single oracle bit $q_j=O_q(e_j)$.
...
Initialize $\sigma=0$. When a strategic coin $v$ opens, update $\sigma$ by
the parity of its already-open strategic neighbours in the queue. Thus an
edge is charged exactly when its two touch intervals overlap. When a front
$v$ closes, update
\[
 \sigma\mathrel{\oplus{=}}d(v),
 \quad d(F_i)=p_i,\quad d(S_j)=q_j.
\]
Passes do not change $\sigma$.
...
\begin{theorem}[split-source exactness]\label{thm:wittfifo}
For every finite-dimensional characteristic-$2$ quadratic refinement $Q_q$,
every input $x$, and either charge stance, split-source \textsc{witt-fifo}
...
\end{equation}
The loaded strategic graph is a matching plus isolates. FIFO implies that an
edge is charged exactly when its two intervals overlap. The odd-close identity
of Section~\ref{sec:linking} says that overlap parity equals total active-edge
parity plus the sum of close charges $\deg_U(f)$. By
Theorem~\ref{thm:matchingfifo}, either seat forces the latter correction to
zero, leaving the last sum in~\eqref{eq:splitsource}. Every active $F_i$ and
$S_j$ closes exactly once, contributing the first two sums. Thus the minimax
charge is $Q_q(x)$ at both stances.
...
\begin{proposition}[naturality]\label{prop:wittfifonatural}
The rule satisfies F1--F3 and N1--N2 with $(w_0,c)=(1,1)$. On every
positive-polar-rank instance it also satisfies N3cf.
\end{proposition}

\begin{proof}
The basis algorithm, $x\mapsto y$, both loaded support sets, the matching
edges, the public charges $p_i$, the OPEN overlap charge, and the terminal
board condition use only $B$ and the fixed frame. Thus F1 holds. Only a close
of $S_j$ accesses the refinement, and it queries exactly $O_q(e_j)$; every
other predicate and transition is refinement-blind. This proves F2 and N2
with $(1,1)$. The semantics is the single normal-play claim rule, giving F3,
and Theorem~\ref{thm:wittfifo} is N1 for every refinement of $B$.

For N3cf choose one symplectic pair $f_a,f_b$ and load $x=f_a+f_b$.
Since $x\ne0$, at least one original-coordinate source $S_j$ is loaded.
OPEN $F_b$ and then every source coin, without closing any. This $q$-blind
prefix reaches a common state for $q$ and $q'=q+e_j^*$ with
\[
 U=\{F_a\},\qquad
 \mathcal Q=(F_b,S_{j_1},\ldots,S_{j_t}),
\]
clear ko, and $S_j$ still unread. The only legal actions are OPEN $F_a$ and
CLOSE $F_b$. OPEN $F_a$ charges the unique active edge because $F_b$ is still
queued; CLOSE $F_b$ makes that overlap impossible. All diagonal contributions
are the same in the two continuations. The terminal bits are therefore
complementary, so the player to move has
exactly one winning action. Toggling $q_j$ complements both bits and exchanges
those actions. Hence the common state is winning in both games and
$W_q(s)\cap W_{q'}(s)=\varnothing$.
\end{proof}

The rank-zero boundary is deliberately not decorated with a fake strategic
edge. There the same rule realizes $Q_q$ and satisfies N1--N2, while N3cf is
inapplicable by its positive-rank clause. Conversely, every nonzero Gold core
exposes form dependence in the optimal strategy, not merely in a terminal
label or a dominated escape option.

N3cf is not claimed to be a metaphysical characterization of naturality: one
could deliberately bolt on a $q$-dependent winning fork. Its proved advantage
is narrower and exact. It repairs the logical impossibility of
dominance-pruned mixed-successor criticality and the dominated escape-edge
attack, while the witness here is the same hyperbolic edge and unread local
source that supply the quadratic term, not an auxiliary outcome gadget.
...
matching class, while split sources preserve fixed-frame locality.
...
(Theorems~\ref{thm:rigidity}, \ref{thm:clock}); the split-source construction
escapes by a q-blind structural lift, not by extra refinement access. Its
matching theorem is uniform in dimension, its transitions are singleton-local,
and its claim lantern gives an ordinary normal-play P-set. Thus Tier 2 is
inhabited by a proved N1+N2+N3cf non-clock.
...
\begin{itemize}[leftmargin=*]
\item \path{formal/Ogdoad/FifoMatching.lean} kernel-checks the both-seat
matching strategy; \path{formal/Ogdoad/GoldMatchingAlgebra.lean} checks the
adapted-support quadratic expansion; and
\path{formal/Ogdoad/GoldSemantics.lean} checks the normal-play compiler,
phase obstruction, dominance-pruning impossibility, and edge-fork swap. These
files contain no \texttt{sorry}, \texttt{admit}, or custom axioms.
...
characteristic-$2$ trace forms inside a nimber Clifford backend, with Arf now
literally the $P$-set bias of split-source \textsc{witt-fifo}. Above it: the
...
matching, split sources preserve original-frame singleton locality, the
both-seat safe-front theorem forces the exact charge in all dimensions, and
the phase-aware claim lantern compiles it to normal play. N3cf repairs the old
anti-clock axiom and is witnessed by an actual swap of optimal actions. The
