...
so any group model realizing $Q$ as its squaring map must be noncommutative;
this yields a useful vocabulary between the frame-blind no-go and direct
global evaluation. Third, the constructive boundary: a public, refinement-blind Witt
...
singleton source pairs attain the sharp support bound. Conversely, the basic
property ``there exists a refinement-sensitive winning fork'', even when the
fork is required after every completed play, cannot certify non-tautology: an
outcome-preserving terminal padding gives every finite normal-play tree that
property. The result is an exact dichotomy
...
kernel-checks the independent all-exponent diagonal-source, matching,
adapted-algebra, compiler-equivalence, observation-lower-bound, and
fork-padding lemmas used in the synthesis; the formal boundary is stated precisely in
...
\begin{remark}[resolved diagonal-framing question]
...
Section~\ref{sec:wittfifo} supplies the formerly missing local normal-play rule
with P-set $\{x:Q_a(x)=0\}$.
\end{remark}
...
\begin{remark}[honest scope: five historical escape hatches]\label{rem:hatches}
Theorem~\ref{thm:nolivemiddle} is relative to its hypotheses, and each
hypothesis exposed an experimental window during the no-go phase:
...
or ``feeding in'' the form was the central definitional question of that
program. Section~\ref{sec:criterion} replaces the vocabulary quarantine by
the invariant transcript-span boundary, and Section~\ref{sec:wittfifo}
attains it with an explicit local rule.
\end{remark}
...
rule constants $q_i$ are game-native via Proposition~\ref{prop:wp}; for every
$a$ they are constructively game-built by
Theorem~\ref{thm:all-diagonal-source}. T2 nevertheless remains a forced clock.
...
Theorem~\ref{thm:forkpadding} explains why these outcome-only fork screens
supply no further theorem of ``non-tautology''.
...
fork padding. \path{formal/Ogdoad/GoldDiagonal.lean} checks the quadratic-tower
trace reconstruction, absolute-trace descent, and finite-field
Artin--Schreier exactness behind the all-exponent source. The paper supplies
the joins between these independently checked
lemmas; there is not a single end-to-end Lean theorem constructing the whole
weighted arena. All six files contain no \texttt{sorry}, \texttt{admit}, or
custom axioms.
...
that the reachable/optimal/unavoidable fork properties cannot certify
naturality. The
