...
through $k=8$ (all $12{,}346$ classes there), with the old two-mode menu's
first failures isolated and the no-self-flip envelope strictly complete;
what remains of it is the general-$n$ induction. The live questions are
\emph{semantics} (recast the charge readout into normal/misère/loopy
outcome classes, or prove the recasting impossible) and the general-$n$
linking proof. The all-$a$ diagonal source is now Theorem
\ref{thm:all-diagonal-source}; sourcing is no longer an open premise.
...
\item[done.] \emph{The even-$a$ diagonal lemma}: resolved by the quadratic-tower
recursion and Artin--Schreier exactness in Theorem
\ref{thm:all-diagonal-source}. This removes the sourcing premise but does
not settle the Tier-2 play-rule reading of the verified realizer.
