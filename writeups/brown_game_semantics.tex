...
question under an explicit synchronized-charge contract.  The later
weighted-source theorem supplies the ordinary quadratic normal-play bit; what
is not claimed here is that a synchronized pair of binary games is itself one
canonical four-class normal-, mis\`ere-, or loopy-play outcome.
\end{abstract}
...
weighted-source Witt--FIFO theorem now supplies precisely that ordinary
quadratic normal-play game.  On the doubled slice $\ell=0$, $q=2Q$, this is
the resolved Gold-form \texttt{tis} construction.
\end{proposition}
...
\BOUNDARY{} Proposition~\ref{prop:game-split} by itself does not turn a forced
...
no independent mod-$8$ charge/output problem left.  The weighted-source
Witt--FIFO theorem supplies the autonomous nonlinear normal-play bit, and
crossing it with the linear bit gives the Brown classes automatically.  This
closes \texttt{over} as a separate invariant/output direction without
asserting that the synchronized pair is already a single canonical
normal/misere/loopy four-way outcome.  If that single-game internalization is
demanded, it is the exact residual boundary; it is not a surviving instance
of the now-resolved binary \texttt{tis} problem.
