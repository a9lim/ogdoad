...
ordinary quadratic normal-play bit. The remaining semantic question is whether
one intrinsic game can realize all four Brown classes without an external
product of binary channels.
\end{abstract}
...
\section{Conclusion and open semantic boundary}
...
Accordingly, under the synchronized four-charge contract there is no second
nonlinear invariant problem: the weighted-source Witt--FIFO theorem supplies
the quadratic bit, and the linear bit supplies the other Brown coordinate.
The exact open boundary is narrower. One may ask for a single fixed game whose
intrinsic four-way outcome realizes $q$, without an external synchronized
product or terminal evaluator. Such a construction must retain the correlation
between $Q$ and $Q+\ell$ in \eqref{eq:walsh-pair}; a no-go theorem must state
the structural axioms it excludes. This is a semantic internalization problem,
not a missing algebraic invariant and not a nonzero ambient Brown colour of all
short-game values.
