...
\begin{abstract}
Over every perfect characteristic-$2$ field with surjective
Artin--Schreier map, every nonsingular finite-dimensional quadratic form is
hyperbolic and the quadratic Witt group is zero.  Conway's full nimber field
$\On$ is algebraically closed of characteristic $2$, so this applies to
$\On$.  A possibly singular form is classified by the dimensions of the
space, its polar radical, and its quadratic radical; there is no further Arf
coordinate.  The Clifford/Brauer--Wall class of every nonsingular form is
therefore split.
...
B(u,v)=Q(u+v)+Q(u)+Q(v).
\]
It is alternating.  Write $R=\rad(B)$, and call $Q$ nonsingular when $R=0$,
as in the characteristic-$2$ terminology of
Elman--Karpenko--Merkurjev~\cite[Ch.~II, \S7]{EKM08}.  Its quadratic radical is
\[
 \rad(Q)=\{r\in\rad(B):Q(r)=0\}.
\]
For the fields considered below this is the kernel of a linear functional on
$\rad(B)$.  We call a characteristic-$2$ field \emph{quadratically closed}
in this note when it is perfect and its Artin--Schreier map is surjective.
...
\[
 ...
 \qquad \F/\wpmap(\F).
\]
For a nonsingular form and a symplectic basis $(e_i,f_i)_{i=1}^r$, the usual
...
identifies it with $\mathbb F_2$. Over the full nimber field no finite
absolute trace exists, so the intrinsic Artin--Schreier quotient is the
...
Both the field theory and the form theory used here are standard; the content
of this note is their application to the full nimber field and the corrective
observation that an Arf bit is relative to a declared subfield and dies in
the colimit.  Conway's nim addition and nim multiplication turn the class of
ordinals into the algebraically closed characteristic-$2$ field $\On$
(\emph{On Numbers and Games}, Theorem~49)~\cite{Conway76}; Lenstra states
this explicitly and analyzes the algebraic-closure beginning
segment~\cite{Lenstra77}.  The surrounding quadratic-form theory is standard;
see also Baeza~\cite{Baeza78} and the characteristic-$2$ treatment of
Hoffmann--Laghribi~\cite{HoffmannLaghribi04}.  Risnoveanu's recent work on
quadratically closed nimber fields is adjacent to, rather than competing
with, the full-field application here~\cite{Risnoveanu26}.

\section{Elementary splitting lemmas}

\begin{lemma}[Quadratically-closed consequences]\label{lem:closure}
Let $\F$ be a quadratically closed characteristic-$2$ field in the sense
above.  Then Frobenius $x\mapsto x^2$ and the Artin--Schreier map
$\wpmap(x)=x^2+x$ are both surjective, and $\F/\wpmap(\F)=0$.
\end{lemma}

\begin{proof}
Perfection makes Frobenius surjective, and Artin--Schreier surjectivity is
part of the hypothesis.  The quotient conclusion is immediate.
\end{proof}

\begin{corollary}\label{cor:algclosed-quadratically-closed}
Every algebraically closed field of characteristic $2$ is quadratically
closed in the sense above.
\end{corollary}

\begin{proof}
For $a\in\F$, algebraic closure supplies roots of $X^2-a$ and of
$X^2+X-a$, respectively proving Frobenius and Artin--Schreier surjectivity.
\end{proof}
...
\begin{lemma}[Every nonsingular plane is hyperbolic]\label{lem:plane}
Let $\F$ be a characteristic-$2$ field with surjective Artin--Schreier map.
Every nonsingular
...
\]
In either case take a nonzero isotropic vector $v$.  Nonsingularity supplies
$w_0$ with $B(v,w_0)=1$.  For every $c\in\F$,
\[
 Q(w_0+cv)=Q(w_0)+c^2Q(v)+cB(w_0,v).
\]
Only now substitute the already established equality $Q(v)=0$ and take
$c=Q(w_0)$.  Then $w=w_0+Q(w_0)v$ satisfies $B(v,w)=1$ and
\[
...
\begin{proposition}[The nonsingular part]\label{prop:nonsingular}
Every nonsingular quadratic space of dimension $2r$ over a
characteristic-$2$ field with surjective Artin--Schreier map is isometric to
$H^{\perp r}$.
Consequently $\Wq(\F)=0$.
\end{proposition}
...
hyperbolic.  Its polar-orthogonal complement is again nonsingular.  Induction
splits off $r$ hyperbolic planes.  Since every nonsingular form is
hyperbolic, its quadratic Witt class is zero.
\end{proof}
...
\section{Complete normal form over quadratically closed fields}

\begin{theorem}[Quadratically-closed characteristic-$2$ classification]
\label{thm:classification}
Let $Q$ be a finite-dimensional quadratic form over a quadratically closed
characteristic-$2$ field $\F$, let $B$ be its polar form, and write
\[
 \rank(B)=2r,
 \qquad \dim\rad(B)=s,
 \qquad
 \epsilon=[\rad(Q)\subsetneq\rad(B)],
\]
where brackets denote the indicator of the displayed condition.  Then
\[
 (V,Q)\cong
 \begin{cases}
 ...
 \end{cases}
\]
Here the second case necessarily has $s\ge1$; if $s\ge2$, its radical
summand $A\perp Z^{\perp(s-1)}$ is isotropic.  Thus two such forms are
isometric if and only if the triples
\[
 \bigl(\dim V,\dim\rad(B),\dim\rad(Q)\bigr)
\]
agree.  In particular every nonsingular form has a unique isometry class at
fixed dimension, its Arf class is zero, and no transfinite Arf bit exists.
\end{theorem}
...
normal forms.  In the proof of Lemma~\ref{lem:radical},
$\rad(Q)=\ker\ell$: thus it has dimension $s$ when $\ell=0$, and dimension
$s-1$ when $\ell\ne0$.  Equivalently,
$\epsilon=[\rad(Q)\subsetneq\rad(B)]$.  These dimensions recover the two
normal forms and are isometry invariants.
\end{proof}

\begin{corollary}[Full nimber field]\label{cor:on2-classification}
The theorem applies to the full nimber field $\On$.  In particular, every
nonsingular finite-dimensional quadratic form over $\On$ is hyperbolic and
$\Wq(\On)=0$.
\end{corollary}

\begin{proof}
Conway's field is algebraically closed of characteristic $2$, so
Corollary~\ref{cor:algclosed-quadratically-closed} applies.
\end{proof}
...
subfield generated by the finitely many coefficients.  Replacement, applied
to its set of polynomials and the finite set of ordinal roots of each,
shows that the relative algebraic closure inside $\On$ is set-sized; the
argument can be carried out there.
\end{remark}
...
remaining anisotropic nonsingular core.
...
Let $K_d=\mathbb F_{2^d}\subset\On$.  For a nonsingular form over $K_d$, its Arf
class lies in
\[
...
extension to $K_{2d}$.  The finite-field antecedent goes back to
Dickson~\cite{Dickson01}.
...
$K_d/\wpmap(K_d)\to K_{2d}/\wpmap(K_{2d})$ is zero.  Arf's 1941
classification theorem~\cite{Arf41} then makes the Witt-group map zero.
Every element at every finite stage
...
\begin{corollary}[The nonsingular Clifford class splits]
\label{cor:clifford}
For every nonsingular finite-dimensional quadratic form $Q$ over $\On$, the
graded Morita class of $\Cl(Q)$ is zero on Ogdoad's characteristic-$2$
...
carried by Clifford algebras of nonsingular quadratic forms---the exact surface
...
The classification has four consequences:
\begin{enumerate}
...
\item \textbf{Root route.}  Algebraic closure proves that the required roots
      ...
      than by mathematics.
\item \textbf{Brauer--Wall.}  Every nonsingular quadratic Clifford class is split;
      singular radicals carry no class of the same kind.
\end{enumerate}

The full-nimber theory therefore closes by collapse, not by a new transfinite
bit. The finite bit remains useful precisely because it remembers a smaller
declared ground field.
...
\bibitem{Baeza78}
R.~Baeza,
\emph{Quadratic Forms over Semilocal Rings},
Lecture Notes in Mathematics 655, Springer, 1978.
...
AMS Colloquium Publications 56, American Mathematical Society, 2008.

\bibitem{HoffmannLaghribi04}
D.~W.~Hoffmann and A.~Laghribi,
\emph{Quadratic forms and Pfister neighbors in characteristic 2},
Transactions of the American Mathematical Society \textbf{356} (2004),
4019--4053.
...
\bibitem{Risnoveanu26}
L.~R\^i\c{s}noveanu,
\emph{Some quadratically closed fields of nimbers},
arXiv:2606.28414, 2026.
