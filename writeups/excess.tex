...
factors; the trace bit itself remains open.  On the singleton-even Fermat
...
original Kummer scalar and supplies no independent relation.
...
Kummer class.  The cubic and exceptional arms also admit a common
...
primary factor above the corresponding threshold.  Parallel partition
bounds hold on the cubic spine.  The remaining small factors in these
...
conjugate-ball factor range &
the selected residue symbol at the remaining smaller primary factors \\
\((C)\) &
\(\gamma_k\) primitive &
proof for \(k\leq3\); partition order bounds; exact derivative
regulator and reciprocity boundary &
nonvanishing at the remaining current primary factors \\
\((D)\) &
\(\Psi_k\mid\ord(M_k)\) &
exact current-factor criterion; projection and antiunit partition
bounds; equivalent polynomial and character formulations &
nonvanishing of the conductor-five antiunit at the remaining current
primary factors \\
\bottomrule
\end{tabular}
\end{center}
...
boundary-character data are circular.  For multicomponent zero sets,
...
Kummer class without evaluating it.  The remaining theorem is direct
nonvanishing at the smaller current factors.
