...
gives a negative answer under an exact naturality condition and explains why
the nonzero Gold forms on nimbers cannot extend to
...
\section{Divisibility of the short-game group}

Let $\D=\Z[1/2]$ be the additive group of dyadic rationals.

\begin{theorem}[Moews]\label{thm:moews}
The additive group of short partizan games has abstract group
structure
\[
...
Let $A$ be an abelian group, $C$ an associative ring, and
...
The kernel-checked file is \texttt{GameExterior.lean}. Its theorem takes the roots
...
Changing the commutative coefficient target cannot rescue a
...
third changes the mathematical object and belongs to directed game semantics,
not to a deformation of \path{GameExterior}. No commutative coefficient target
evades the theorem.
