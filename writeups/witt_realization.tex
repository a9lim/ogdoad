...
Ramified Residue Coordinates and Impartial Realization Boundaries}
...
normal-play values for every nonsingular Witt class.  We derive the full
finite triangular coordinate transformation under a ramified extension
$\pi=u\varpi^e$, including its trace-character adjoint.

The stronger request for one fixed finite refinement-independent local-access
family is impossible already for binary planes: the refinements
$[1,P^{-1}]$, with $P$ irreducible, have one fixed polar form and infinitely
many distinct place coordinates.  Thus a finite sparse family must either
select its labels from the refinement, becoming a transducer, or be replaced
by an infinite on-demand family.  Finally, no additive impartial invariant
can extend faithfully to arbitrary singular quadratic spaces: the quasilinear
relation
...
This paper extracts the exact normal-play consequence of that sequence,
computes ramified scalar extension on the resulting coordinates, and settles
two stronger requests negatively: a fixed finite local-access family cannot
cover the full refinement torsor, and arbitrary quasilinear radical data
cannot survive an additive game law.
...
of the odd-pole coefficients, functorial in towers and adjoint on trace labels.
...
ramified transformation, and both impossibility theorems are proved here.
...
\section{Ramified coordinate change}\label{sec:ramification}

We now compute the coordinate transformation that was left implicit in the
classification-to-game compiler.  Let
\[
...
There is a necessary qualification concerning the word ``transfer.''  The
...
\section{The finite local-access obstruction}\label{sec:finite-obstruction}
...
place and pole label can keep refinement-independent boards, but it is
infinite, although each class has finite nonzero support.  Selecting only that
support produces a finite family, but the selection depends on the
refinement and is therefore a sparse classifier or transducer.  Restricting
the input to a fixed finite set of places and a bounded conductor also yields
a finite static family.  These are coherent relaxations; none satisfies the
literal conjunction ruled out by the theorem.
...
The Ogdoad Lean modules \texttt{WittRealizationExact} and
\texttt{WittRamification} formalize the parts of the proof that do not require
a quadratic Witt-group library \cite{ogdoad2026}.  Given linear maps fitting
the exact-sequence hypotheses, the first proves:
\begin{itemize}
...
The second module represents a finite principal part by pole order and proves:
\begin{itemize}
\item an elementary Artin--Schreier move kills one even top pole and changes
only a strictly lower order;
\item descending normalization terminates, removes every bounded positive
even pole, preserves the constant coefficient, and is triangular in pole
order;
\item every normalization is connected to its input by a finite chain of
exact Artin--Schreier moves; and
\item no map from an infinite class family to a fixed finite P/N outcome vector
is injective.
\end{itemize}

Lean does not formalize the Aravire--Jacob sequence, the local quadratic Witt
groups, Lemma~\ref{lem:transfer}, the unit-series substitution in
Theorem~\ref{thm:ramified}, or the infinitude of the rank-two family; those
remain paper-level mathematics grounded in the cited exact sequence and
finite-field theory.  The Rust local decomposition computes the triple
$(\phi_0,\psi,\phi_1)$ with canonical uniformizers.  It does not yet expose the
global coordinate theorem as a public Witt-class API or implement ramified
coordinate transport.

\section{Conclusion}

The nonsingular realization problem has a complete classification answer.
The infinite unramified coordinate and all second residues give canonical
coordinates for $W_q(\F_2(t))$.  Finite trace characters present the wild
odd-pole terms basis-freely, and the games $0$ and $*$ realize the resulting
finite-support bit vector exactly.  Reciprocity is one parity equation on the
ramified Arf bits.  Under ramified scalar extension, the unit jet and finite
inverse-Frobenius recurrence give the exact triangular coordinate law.
...
coordinate family, a refinement-dependent sparse transducer, or a bounded
...
rules out the additive singular extension.  Both obstructions are algebraic
and precede any choice of game rules.
