...
Transition Traces, Tame Octal Realizations, and Grundy's Game}
...
\begin{abstract}
Siegel's valid-transition-table theorem characterizes finite mis\`ere
quotients abstractly, but it does not decide which quotients arise from a
uniform heap ruleset.  We isolate the missing homogeneity condition as an
exact quotient-valued trace recurrence.  A least-rank separation argument
shows that every valid transition table on a reduced bipartite monoid is
functional over option-value sets.  Consequently every finite no-split
octal code with finite quotient has an ultimately periodic pretending
function, with an effective finite-state bound.  Ultimately periodic split
traces also admit finite exact certificates, although periodicity is no
longer automatic.

We then give a uniform natural realization.  For every $n\ge2$, the finite
subtraction code with $2^{n-1}$ consecutive digits equal to $3$ has exact
mis\`ere quotient $\mathcal T_n$, the tame quotient of order $2^n+2$.  The
proof supplies the quotient map for every heap and a direct winning strategy
for every sum.  Thus the complete tame arm in Siegel's finite
$|P|=2$ classification is naturally realized by finite codes.

Finally we begin the exact analysis of mis\`ere Grundy's game.  Its quotient
through heap $12$ is $\mathcal T_2$; heap $13$ causes the first extension,
to a reduced quotient of order $12$, and heap $18$ extends this to a reduced
quotient of order $24$.  These are symbolic all-multiplicity results, not
bounded indistinguishability computations.  The full quotient and the
uniform realization of the second $|P|=2$ family remain open.
\end{abstract}

\section{The natural realization problem}
...
The natural problem begins after that theorem.  A finite octal code is not
an arbitrary finite list of games: each code digit acts on every later heap.
The resulting translation-invariant tail is the additional structure that
must be characterized.

\section{Valid tables are deterministic}
...
\section{No-split codes are finite-state}
...
\begin{thmA}[Automatic periodicity]
...
\section{A natural realization of every tame quotient}
...
\begin{thmB}[Tame-family realization]\label{thm:tame}
...
\section{Exact initial quotients of mis\`ere Grundy's game}
...
\begin{thmC}[Exact Grundy prefix]\label{thm:grundy18}
...
The Lean development currently checks the load-bearing transition
