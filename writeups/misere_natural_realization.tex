...
Exact Octal Traces, Heap Normal Forms, and Grundy's Game}
...
must generate the table at every heap size.  We formulate that requirement as
a canonical quotient-valued heap trace.  In every reduced valid table the
complete option-value set determines its value, and no transition value can
occur among its own options.  These facts give an if-and-only-if trace
criterion, absolute digit obstructions, automatic periodicity for finite
no-split quotients, and finite certificates for asserted periodic split
traces.  The split recurrence is exactly a system of unary Boolean language
equations.  For arbitrary partial decoders its totality problem is
$\Pi^0_1$-complete already for $0.7$; the reduction deliberately falls
outside the valid-table class and therefore isolates, rather than resolves,
the remaining decision problem.

On the realization side, every nontrivial finite valid quotient has a
one-species heap model with finitely many source-local rules and the uniform
tail $H_n\to H_{n-1}$, but an inert-pad theorem prevents a verbatim finite-octal
encoding when the quotient has more than two elements.  Positively, the code
with $2^{n-1}$ consecutive digits $3$ realizes the tame quotient
$\mathcal T_n$ for every $n\ge2$.  For mis\`ere Grundy's game we prove exact
all-multiplicity quotients of orders $6$, $12$, and $24$ through heaps $12$,
$13$, and $18$.  If the full quotient is finite, its kernel is nontrivial.
The wild octal-realization family and the full Grundy quotient remain open.
\end{abstract}
...
The argument has one spine.  Transition-table determinism makes the heap word
canonical; the octal rule turns that word into a totality problem; exact
periodic certificates and normal forms locate the boundary of the problem;
and the tame and Grundy calculations solve the recurrence in two complementary
regimes.  The main results are grouped accordingly.
...
and generates the monoid.  No transition value is one of its own option
values, yielding explicit whole-, one-, and two-remainder exclusions.
\item Finite no-split quotients have ultimately periodic heap traces, so exact
realization by a fixed no-split code is decidable.  An asserted periodic split
trace also has a finite exact certificate.  For arbitrary partial decoders,
however, fixed-code split totality is $\Pi^0_1$-complete already for $0.7$;
the valid-table case remains open.
\item Every nontrivial finite quotient has a finite-exception unary-heap normal
form with tail $H_n\to H_{n-1}$.  Its inert pad cannot be retained by a
verbatim finite-octal encoding when $|Q|>2$.  In the strict octal class, for
every $n\ge2$ the code
\(0.\underbrace{33\cdots3}_{2^{n-1}\text{ digits}}\) has exact quotient
$\mathcal T_n$, realizing every tame quotient in the finite $|P|=2$
classification.
...
The normal form in item~3 belongs to a comparison class, while the tame family
in the same item is a strict finite-octal theorem.  Item~4 is an exact prefix
theorem plus a conditional structure theorem.  None of the four items asserts
automatic periodicity for valid split traces or finiteness of the full Grundy
quotient.
...
\section{Authority and exact frontier}\label{sec:boundary}

The abstract classification theorem is Siegel's cited result.  The new claims
in this paper have the authority recorded in Table~\ref{tab:misere-status}.
The distinctions are substantive: a bounded quotient signature is not an
exact infinite-ruleset certificate, and a Lean definition of a transition
target is not a proof that a natural ruleset realizes it.
...
Status & Content \\
\midrule
Cited input & Plambeck--Siegel quotient theory, Siegel's valid-transition-table
classification, the stated Grundy--Smith global reductions, and the unary
conjunctive-grammar hardness theorem. \\
Proved here & Option-set determinism; the exact trace criterion and digit
exclusions; no-split periodicity and periodic split certificates; arbitrary-
decoder hardness; the finite-exception normal form and inert-pad obstruction;
the tame-family realization; the exact Grundy quotients through heap $18$;
and the conditional kernel theorem. \\
Kernel-checked & Transition determinism, meximality, $P$-reaching contexts and
no-self; exact quotient fibers; octal record periodicity and tail
certification; the typed prefix/pad/unary-tail construction; the inert-pad
obstruction; the unary-language dictionary; and the finite-color and
power-period cores of the Grundy obstruction. \\
Paper-level bridge & The grammar reduction itself; numerical enumeration and
multiset transport for the typed normal form; the tame all-multiplicity
strategy; the Grundy presentations and translation rows; and the final
minimal-ideal deduction in Theorem~\ref{thm:grundy-kernel}. \\
Open & Fixed valid-table split totality, unbounded finite-code synthesis, the
tame extensions of $\mathcal R_8$, and the exact full quotient of mis\`ere
Grundy's game. \\
\bottomrule
\end{tabular}
\caption{Claim authority for the natural-realization paper.}
\label{tab:misere-status}
\end{table}

The import-only entry point \texttt{MisereNaturalRealization} contains no
parallel paper-specific copy of the transition calculus.  The theorem owners
are separated by role:
\begin{itemize}
  \item \texttt{MisereTransition}: transition algebra and least-rank
        determinism;
  \item \texttt{MisereOctalCertificate}: finite octal certificates;
  \item \texttt{MisereNaturalUniversality}: typed realization and normal
        forms;
  \item \texttt{MisereTraceLanguage}: the partial-trace reduction; and
  \item \texttt{MisereGrundyObstruction}: exact Grundy presentations and the
        kernel obstruction.
\end{itemize}
The entry point imports only the terminal owners needed to expose this
dependency graph.

The remaining problem has two independent octal layers and one Grundy layer.
First, for fixed finite $(Q,P)$, valid table $T$, and split code $\Gamma$,
decide whether the canonical partial trace is total.  The arbitrary-decoder
$\Pi^0_1$ theorem shows why a proof must use validity, but gives no hardness
result for valid tables.  Second, even a fixed-code decision theorem does not
bound the length of a synthesizing code.  A monoid-level characterization
therefore needs a computable pumping or length theorem, or an undecidability
proof for the synthesis problem.  Table enumeration adds no further gap,
because only finitely many transition tables exist on fixed $Q$.

For $|P|=2$, the tame arm is complete and the tame extensions of
$\mathcal R_8$ remain.  For Grundy's game, a complete solution must either
continue the exact presentations and prove stabilization, or exhibit
infinitely many distinct translation rows with explicit separating contexts.
In the finite branch, Theorem~\ref{thm:grundy-kernel} requires a nontrivial
kernel and a nonconstant eventual power cycle.

\section{Conclusion}

Natural realization is a totality and synthesis problem for one canonical
quotient-valued heap trace.  No-split codes are finite-state; split codes add
the unbounded convolution measured by the unary-language equations.  The
finite-exception normal form shows that an infinite uniform tail is not by
itself the obstruction.  Strict octal homogeneity is: one digit couples an
intended prefix move to infinitely many translated moves.

That formulation supports exact positive and negative results.  The no-self
and inert-pad theorems reject impossible direct encodings, while one explicit
family realizes every tame quotient.  Grundy's game supplies exact wild
prefixes at heaps $13$ and $18$, but no stabilization or infinite separation
theorem yet follows.  The next decisive result must control the valid split
recurrence or the Grundy extension process uniformly; further bounded tables
alone cannot close either problem.
