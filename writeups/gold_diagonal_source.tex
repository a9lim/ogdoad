...
\]
the diagonal constants $Q_a(e_i)$ are the datum missing from the polar form.
Previously a uniform game-built source was known only for $a=1$, while the
trace-dual elements for even $a$ visibly drifted under tower doubling.  We
give a recursion for the trace-dual diagonal of every scaled Gold component,
...
\]
The result closes the diagonal-sourcing problem, including the even-$a$
case.  It does not solve the separate problem of turning those constants into
a non-clock normal-, mis\`ere-, or loopy-outcome game.
\end{abstract}

\section{Statement and boundary}
...
The theorem is about sourcing the diagonal constants.  It does not infer a
$P$-set theorem, a linking strategy, or a resolution of the full natural
Gold-quadric game problem.  This proof/semantics boundary is essential.
...
This is the point at which the even-$a$ drift ceases to be an obstruction.
The scale $d_1$ can change at every tower level; the recursion transports that
change instead of requiring one fixed dual element throughout the tower.
...
\section{Consequence for the open Gold-game program}

The former even-$a$ constructive sourcing question is closed: for every
exponent, the diagonal is supplied by one deterministically selected tower
...
What remains open is the strategic layer.  A rule may consume the sourced
diagonal and still be a transport clock, or may realize only a terminal charge
rather than a normal/mis\`ere/loopy outcome class.  The all-$a$ source removes
one algebraic obstruction from that problem; it does not weaken the existing
naturality and outcome-semantics requirements.
