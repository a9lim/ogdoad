...
into queued four-cycles.  Separately, every close-first attacker is contracted
...
scalar score.  If $U_\lambda$ is the untouched set at an exit $\lambda$, its
future disjointness increment on the deleted pair is exactly
\begin{equation}\label{eq:kappa-front}
 \kappa_{xy}(U_\lambda)
 =\sum_{u\in U_\lambda}(xu+yu).
\end{equation}
This chain is Eulerian and $B$-isotropic:
\[
 \partial\kappa_{xy}(U_\lambda)=0,
 \qquad
...
the queue overlaps them.  This proves \eqref{eq:kappa-front}.  The residual
set has even cardinality, so every vertex in the displayed complete bipartite
chain has even degree.  Pairing that chain with $B$ gives the same even
$\eta$-sum, proving the last two assertions.
\end{proof}
...
\item \OPEN{} The global FIFO affine-contraction statement
      ...
      four-cycle defects across branches.
\end{itemize}
