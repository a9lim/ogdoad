...
| canonical nim multiplication | `NimFastMultiplication` | The finite-field trace-one irreducibility criterion, trace carry through a quadratic generator, the three-variable-product quadratic split, the affine Artin--Schreier generator shift, its involutive two-block coordinate transform, preservation of the canonical tower equations under ring equivalence, and exact recovery of source-basis product coordinates. The concrete Conway tower, De Feo--Schost algorithms, and asymptotic bounds remain cited or paper-level inputs. |
