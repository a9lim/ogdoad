...
| imperfect-field Witt reduction | `WittRealizationExact` | An exact residue sequence plus a constant detector gives canonical coordinates `W ≃ C × ker(transfer)`; finite-field trace characters give a basis-free faithful bit presentation; cancellation rules out a faithful additive target for arbitrary quasilinear radical data. The Aravire--Jacob exact sequence, its infinite-place detector, finite-field transfer calculation, and ramified base-change law remain paper-level or external inputs. |
