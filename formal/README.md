...
| imperfect-field Witt reduction | `WittRealizationExact`, `WittRamification` | An exact residue sequence plus a constant detector gives canonical coordinates `W ≃ C × ker(transfer)`; finite-field trace characters give a faithful bit presentation; finite triangular Artin--Schreier reduction eliminates even poles without changing the constant; and an infinite class family cannot inject into a fixed finite P/N vector. The Aravire--Jacob exact sequence, its infinite-place detector, finite-field transfer calculation, unit-series ramified substitution, and infinitude of the rank-two place family remain paper-level or external inputs. |
